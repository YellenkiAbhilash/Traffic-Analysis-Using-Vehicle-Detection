...
<body>
    <div class="dashboard">
        <h1>Traffic Dashboard for B</h1>

        <h1>Vehicle Detection Stream for Region B</h1>
        <img src="http://localhost:5000/video_feed_b" width="1020" height="500" alt="Region B Vehicle Detection Stream">
        ...
        <ul id="emergency-alerts-list">
            <?php
            $query = "SELECT * FROM emergency_alerts_b ORDER BY created_at DESC";
        ...
        <ul id="alerts-list">
            <?php
            $query = "SELECT * FROM alerts_b WHERE region = 'B' ORDER BY created_at DESC";
            $result = mysqli_query($conn, $query);
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<li>' . htmlspecialchars($row['message']) . 
    ...
    <script>
        ...
        function fetchEmergencyAlerts() {
            fetch('get_emergency_alerts_b.php')
                .then(response => response.json())
                .then(data => {
